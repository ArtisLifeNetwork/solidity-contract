...
contract ArtisFarmAngelClub is IERC777Recipient {
    using SafeMath for uint256;
    mapping(address => stakerObject) public stakers;
    uint256 public rate; //time length for interest (default 1-year)
    uint256 public interestAPR; //amount of interest APR 0 - 100
    ...
    string public name = "ArtisLife Network Angel Club Farm";
    ...
    ERC1155 public angelClubNFT;
    uint256 public angelClubNFTid;
    ...
    constructor() {
        ...
        angelClubNFT = ERC1155(0xa79dF500990cBEf47c4b0b7B8c70666bd2fe389b);
        angelClubNFTid = 0;
    ...
    */
    function tokensReceived(
        ...
        require(
            angelClubNFT.balanceOf(from, angelClubNFTid) > 0,
            "Not Angel Club Member."
        );
    ...
    function compoundYield(address staker) private {
        ...
        require(angelClubNFT.balanceOf(staker, angelClubNFTid) > 0, "Not Angel Club Member.");
    ...
    function compoundBatch(address[] memory _stakers) external onlyAdmin {
        for (uint256 i = 0; i < _stakers.length; i++) {
            if (angelClubNFT.balanceOf(_stakers[i], angelClubNFTid) > 0)
                compoundYield(_stakers[i]);
            else _unstake(stakers[_stakers[i]].artisBalance, _stakers[i]);
        }
    }
    ...
    function _unstake(uint256 amount, address account) private {
        require(
            stakers[account].isStaking =
                true &&
                stakers[account].artisBalance >= amount,
            "Nothing to unstake"
        );
        uint256 yield = calculateYieldTotal(account);
        totalARTISRewards = totalARTISRewards.sub(yield);
        totalARTISLocked = totalARTISLocked.sub(amount);
        stakers[account].startTime = block.timestamp;
        stakers[account].artisBalance = stakers[account].artisBalance.sub(
            amount
        );
        artisToken.send(account, amount.add(yield), "");
        if (stakers[account].artisBalance == 0) {
            stakers[account].isStaking = false;
        }
    ...
    function setAngelClubNFTid(uint256 newID) external onlyAdmin {
        angelClubNFTid = newID;
    }
